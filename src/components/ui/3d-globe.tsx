...
export interface GlobeMarker {
  lat: number;
  lng: number;
  /** Used as dot color accent or legacy image src – keep it a hex color like '#ef4444' */
  src: string;
  /** City / exchange name shown in the popup */
  label?: string;
  /** Main news / content shown on hover */
  content?: string;
  size?: number;
}
          ...
          >
            <div
              style={{
                ...
                borderRadius: "50%",
                background: hovered ? "#f97316" : "#60a5fa",
                boxShadow: hovered
                  ? "0 0 10px 4px rgba(249,115,22,0.7)"
...
export function Globe3D({
  ...
  return (
    <div className={cn("relative h-[720px] w-full", className)}>
      <Canvas
        gl={{
          antialias: true,
          alpha: true,
